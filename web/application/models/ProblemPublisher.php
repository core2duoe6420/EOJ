...
class EOJ_Model_ProblemPublisher extends EOJ_Model_Adminstrator
{
	...
	public function UpdateUploadedProblem($iuploader_id,$ip_title,$ip_desc,$ip_lang,$ip_tlimt,$ip_mlimt,$ip_input_tips,$ip_output_tips,$ip_sampleinput,$ip_sampleoutput,$ip_hint,$ip_specjg){
		//
		$connection=mysql_connect("localhost","eojapp","ecust")
		or die("Couldn't connect to server");
		$db=mysql_select_db("eojdb",$connection)
		or die("Couldn't select database");
		//
		$callsql="call UPDATEUP('$iuploader_id','$ip_title','$ip_desc','$ip_lang','$ip_tlimt','$ip_mlimt','$ip_input_tips','$ip_output_tips','$ip_sampleinput','$ip_sampleoutput','$ip_hint','$ip_specjg')";
		$result = mysql_query($callsql, $connection) or die("daodiyoumeiyouyong:".mysql_error());
		$row=mysql_fetch_array($result);
		$oec=$row['oexitcode'];
		//
