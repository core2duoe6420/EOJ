...
proc6: BEGIN
  ...
  BEGIN
    ...
    VALUES(2,CONCAT('failed add a eojuser record ','When meet a mysql sql error'),1);
	COMMIT;
  END;
  ...
  THEN
	...
	-- generate log
    INSERT INTO eojdb.EOJLOG(opcode, opmesg, op_tag)
    VALUES(3,CONCAT('failed because of a null username'),2);
	COMMIT;
  ...
  -- generate log
  INSERT INTO eojdb.EOJLOG(opcode, opmesg, op_tag)
  VALUES(1,CONCAT('successful add a eojuser record that [user_id] ',ouser_id),0);
  COMMIT;
  /* end execution */
END
//
